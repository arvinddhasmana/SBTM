...
export const mockOrganizationApi = {
  ...
  createBoard: async (payload: CreateBoardPayload): Promise<Board> => ({
    id: `BRD-${Math.random().toString(36).substr(2, 6)}`,
    ...payload,
  }),
  ...
  listSchools: async (boardId?: string): Promise<School[]> =>
    boardId ? MOCK_SCHOOLS.filter((s) => s.boardId === boardId) : MOCK_SCHOOLS,
  getSchool: async (id: string): Promise<School> =>
    MOCK_SCHOOLS.find((s) => s.id === id) || MOCK_SCHOOLS[0],
  createSchool: async (payload: CreateSchoolPayload): Promise<School> => ({
    id: `SCH-${Math.random().toString(36).substr(2, 6)}`,
    ...payload,
  }),
  updateSchool: async (id: string, payload: UpdateSchoolPayload): Promise<School> =>
    ({ ...MOCK_SCHOOLS[0], ...payload, id }) as School,
