...
const Sidebar: React.FC<SidebarProps> = ({ width, isCollapsed, onToggleCollapse }) => {
    ...
    return (
        ...
        >
            {/* Logo */}
            <div className={`p-6 border-b border-dashboard-border w-full flex items-center ${isCollapsed ? 'justify-center p-4' : 'gap-3'}`}>
            ...
            {/* Navigation */}
            <nav className={`flex-1 p-4 space-y-1 w-full overflow-y-auto custom-scrollbar ${isCollapsed ? 'px-2' : ''}`}>
                {navItems.map((item) => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        className={({ isActive }) =>
                            `flex items-center gap-3 rounded-xl transition-all duration-200 group ${isCollapsed ? 'justify-center p-3' : 'px-4 py-3'
                            } ${isActive
                    ...
                    >
                        ...
                        {!isCollapsed && <span className="font-medium whitespace-nowrap animate-in slide-in-from-left-2">{item.label}</span>}
                    </NavLink>
                ))}
            </nav>

            {/* User & Logout */}
            <div className={`p-4 border-t border-dashboard-border w-full space-y-2 ${isCollapsed ? 'px-2' : ''}`}>
                <button
                ...
                <button
                    ...
                    className={`flex items-center gap-3 w-full rounded-xl text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200 ${isCollapsed ? 'justify-center p-3' : 'px-4 py-3'
                        }`}
