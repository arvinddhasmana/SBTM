...
const PresenceCard: React.FC<PresenceCardProps> = ({ student, onClick }) => {
    ...
    return (
        ...
        >
            <div
                className={`w-6 h-6 rounded flex items-center justify-center ${isBoarded
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                    <p className="text-[10px] font-black text-white tracking-tighter truncate leading-none">{student.name}</p>
                    <div className="flex items-center gap-1.5 ml-2">
                        <div className="flex items-center gap-0.5 px-1 bg-blue-500/10 rounded text-[7px] font-black text-blue-400 uppercase">
                            <RouteIcon size={8} />
                            <span>{student.routeId?.split('-').pop()}</span>
                        </div>
                    </div>
                </div>
