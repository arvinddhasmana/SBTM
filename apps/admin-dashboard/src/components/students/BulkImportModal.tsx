    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <Card className="w-full max-w-xl relative overflow-hidden">
        ...
        <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
          ...
          {t('students:bulkImportModal.title')}
        </h2>
        ...
        <div className="space-y-6">
            ...
            <label htmlFor="csv-upload" className="flex flex-col items-center gap-4 cursor-pointer">
              ...
              <div>
                <p className="text-white font-medium">
                  {file ? file.name : t('students:bulkImportModal.clickToSelect')}
                </p>
                <p className="text-sm text-slate-500 mt-1">
                  {t('students:bulkImportModal.csvFormat')}
                </p>
              </div>
            </label>
          </div>
            ...
            >
              <div className="flex items-center gap-2 font-medium">
                ...
                <span className={result.failed === 0 ? 'text-green-500' : 'text-orange-500'}>
                  {t('students:bulkImportModal.importComplete')}
                </span>
              </div>
              <p className="text-sm text-slate-300">
                {t('students:bulkImportModal.successImported', { count: result.success })}
                {result.failed > 0 &&
                  t('students:bulkImportModal.failedCount', { failed: result.failed })}
              </p>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            >
              {t('students:bulkImportModal.cancel')}
            </button>
            <button
            ...
            >
              {isUploading
                ? t('students:bulkImportModal.uploading')
                : t('students:bulkImportModal.startImport')}
            </button>
          </div>
        </div>
      </Card>
