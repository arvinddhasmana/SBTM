    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <Card className="w-full max-w-lg relative">
        ...
        <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
          ...
          {t('students:routeAssignmentModal.title')}
        </h2>
          ...
          <div className="space-y-6">
            <div className="p-4 bg-slate-800/50 rounded-xl border border-dashboard-border">
              ...
              <p className="text-sm text-slate-400">
                {student?.grade} • {student?.school_id.substring(0, 8)}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">
                  {t('students:routeAssignmentModal.amRoute')}
                </label>
                <select
                  value={amRoute}
                ...
                >
                  <option value="">{t('students:routeAssignmentModal.noAmRoute')}</option>
                  {routes.map((r) => (
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">
                  {t('students:routeAssignmentModal.pmRoute')}
                </label>
                <select
                  value={pmRoute}
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-dashboard-bg border border-dashboard-border text-white text-sm focus:border-primary-500 transition-colors"
                >
                  <option value="">{t('students:routeAssignmentModal.noPmRoute')}</option>
                  {routes.map((r) => (
                    <option key={r.id} value={r.id}>
                      {r.name}
                    </option>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              >
                {t('students:routeAssignmentModal.cancel')}
              </button>
              <button
                onClick={handleSave}
                disabled={isSaving}
              ...
              >
                <Save size={18} />
                {isSaving
                  ? t('students:routeAssignmentModal.saving')
                  : t('students:routeAssignmentModal.saveButton')}
              </button>
            </div>
          </div>
        )}
