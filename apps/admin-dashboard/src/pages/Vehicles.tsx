...
const Vehicles: React.FC = () => {
    ...
    const fetchVehicles = async () => {
        try {
            const data = await fleetApi.getAllVehicles();
            setVehicles(data);
        } catch (error) {
                                    ...
                                    <tr key={vehicle.id} className="group hover:bg-slate-800/30 transition-colors">
                                        <td className="py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-medium text-white">Bus #{vehicle.id.slice(0, 4)}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="py-4 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleOpenModal(vehicle)} className="p-2 text-slate-400 hover:text-primary-400 transition-colors">
                                                    <Edit2 size={18} />
                                                </button>
                                                <button onClick={() => handleDelete(vehicle.id)} className="p-2 text-slate-400 hover:text-red-400 transition-colors">
                                                    <Trash2 size={18} />
                                                </button>
                                            </div>
                                        </td>
