                ...
                <div className="flex gap-4 p-1 bg-dashboard-card border border-dashboard-border rounded-xl w-fit">
                    {(['drivers', 'inspections', 'audit'] as const).map((tab) => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-lg text-sm font-bold transition-all capitalize ${activeTab === tab
                    ...
                    <div className="space-y-6 animate-in fade-in duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <Card className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-2xl font-bold text-white">{drivers.filter(d => d.status === 'VALID').length}</p>
                            ...
                            <Card className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-2xl font-bold text-white">{drivers.filter(d => d.status === 'EXPIRING_SOON').length}</p>
                            ...
                            <Card className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-2xl font-bold text-white">{drivers.filter(d => d.status === 'EXPIRED').length}</p>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-dashboard-border">
                                            ...
                                            <th className="py-4 px-4 text-sm font-bold text-slate-400">Background Check</th>
                                    ...
                                    <tbody className="divide-y divide-dashboard-border">
                                        {drivers.map((driver) => (
                                            <tr key={driver.id} className="hover:bg-white/5 transition-colors">
                                                <td className="py-4 px-4 text-sm text-white font-medium">{driver.driver_id}</td>
                                                <td className="py-4 px-4 text-sm text-slate-300">{new Date(driver.license_expiry).toLocaleDateString()}</td>
                                                <td className="py-4 px-4 text-sm text-slate-300">{new Date(driver.background_check_last_date).toLocaleDateString()}</td>
                                                <td className="py-4 px-4 text-sm text-slate-300">{new Date(driver.medical_check_due_date).toLocaleDateString()}</td>
                                                <td className="py-4 px-4">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${driver.status === 'VALID' ? 'bg-green-500/20 text-green-500' :
                                                            driver.status === 'EXPIRING_SOON' ? 'bg-yellow-500/20 text-yellow-500' :
                                    ...
                                    <tbody className="divide-y divide-dashboard-border">
                                        {inspections.map((ins) => (
                                            <tr key={ins.id} className="hover:bg-white/5 transition-colors">
                                                <td className="py-4 px-4 text-sm text-white">{new Date(ins.createdAt).toLocaleString()}</td>
                                                ...
                                                <td className="py-4 px-4">
                                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${ins.is_passed ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'
                                                        }`}>
                                    ...
                                    <tbody className="divide-y divide-dashboard-border">
                                        {auditLogs.map((log) => (
                                            <tr key={log.id} className="hover:bg-white/5 transition-colors">
                                                <td className="py-4 px-4 text-sm text-slate-400">{new Date(log.createdAt).toLocaleString()}</td>
                                                <td className="py-4 px-4 text-sm text-white">{log.user_id}</td>
                                                <td className="py-4 px-4 text-sm font-bold text-primary-400">{log.action}</td>
                                                <td className="py-4 px-4 text-sm text-slate-300">{log.resource}</td>
                                                <td className="py-4 px-4 text-sm text-slate-400 max-w-xs truncate">{JSON.stringify(log.details)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
