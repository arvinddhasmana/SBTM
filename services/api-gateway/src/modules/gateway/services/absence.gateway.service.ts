...
export class AbsenceGatewayService {
     ...
     */
    async reportAbsence(dto: ReportAbsenceDto, caller: CallerContext): Promise<StudentAbsence> {
        if (!caller.schoolId) {
            throw new ForbiddenException('Parent account is not associated with a school');
        }
        ...
        if (existing) {
            throw new ConflictException('An absence for this student on this date and route type has already been reported');
        }
    ...
    private resolveCallerSchoolId(caller: CallerContext, requestedSchoolId?: string): string {
        if (caller.role === Role.SCHOOL_ADMIN || caller.role === Role.DRIVER || caller.role === Role.PARENT) {
            if (!caller.schoolId) {
