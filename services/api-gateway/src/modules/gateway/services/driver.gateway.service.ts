...
export class DriverGatewayService {
     ...
     */
    async getRouteStudents(routeId: string, user: DriverUser): Promise<RouteRosterStudentDto[]> {
        ...
        let presenceByStudentId = new Map<string, { status: 'BOARDED' | 'ALIGHTED'; lastSeen: string }>();
        try {
            ...
            const presenceList: Array<{ studentId: string; status: string; lastSeen?: string }> =
                Array.isArray(presenceResponse) ? presenceResponse : presenceResponse?.students ?? [];
