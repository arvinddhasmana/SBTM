...
export class ProvisioningService {
     ...
     */
    async inviteUser(dto: InviteUserDto, inviter: InviterUser): Promise<{ message: string; invitationUrl: string }> {
    ...
    async activateAccount(dto: ActivateAccountDto): Promise<{ message: string }> {
        const user = await this.userRepository.findOne({ where: { invitationToken: dto.token } });
     ...
     */
    async listUsers(caller: InviterUser): Promise<Omit<User, 'passwordHash' | 'invitationToken' | 'invitationExpiresAt'>[]> {
        let users: User[] = [];

        if (caller.role === Role.OSTA_ADMIN) {
            users = await this.userRepository.find();
        } else if (caller.role === Role.BOARD_ADMIN && caller.boardId) {
            users = await this.userRepository.find({ where: { boardId: caller.boardId } });
        } else if (caller.role === Role.SCHOOL_ADMIN && caller.schoolId) {
            users = await this.userRepository.find({ where: { schoolId: caller.schoolId } });
        } else {
            throw new ForbiddenException('Insufficient scope to list users');
        }

        return users.map(({ passwordHash: _ph, invitationToken: _it, invitationExpiresAt: _ie, ...rest }) => rest);
    }

    /**
     * Deactivate a user account (soft-disable). Admins can only deactivate users in their scope.
     */
    async deactivateUser(targetUserId: string, caller: InviterUser): Promise<{ message: string }> {
     ...
     */
    async reactivateUser(targetUserId: string, caller: InviterUser): Promise<{ message: string }> {
        const target = await this.userRepository.findOne({ where: { id: targetUserId } });
        if (!target) throw new NotFoundException('User not found');
    ...
    private validateInviterScope(dto: InviteUserDto, inviter: InviterUser): void {
        if (inviter.role === Role.BOARD_ADMIN) {
            ...
            if ((dto.role as Role) === Role.BOARD_ADMIN) {
                throw new ForbiddenException('Board admins cannot create admins above their scope');
            }
        }
        if (inviter.role === Role.SCHOOL_ADMIN) {
            if (dto.role !== Role.DRIVER && dto.role !== Role.PARENT) {
                throw new ForbiddenException('School admins can only invite drivers and parents');
            }
        }
    }

    private resolveSchoolId(dto: InviteUserDto, inviter: InviterUser): string | undefined {
        // School-scoped roles must have a schoolId
        if ([Role.SCHOOL_ADMIN, Role.DRIVER, Role.PARENT].includes(dto.role as Role)) {
    ...
    private resolveBoardId(dto: InviteUserDto, inviter: InviterUser): string | undefined {
    ...
    private assertCallerCanManageUser(target: User, caller: InviterUser): void {
        ...
        if (caller.role === Role.BOARD_ADMIN && target.boardId === caller.boardId) return;
        if (caller.role === Role.SCHOOL_ADMIN && target.schoolId === caller.schoolId) return;
