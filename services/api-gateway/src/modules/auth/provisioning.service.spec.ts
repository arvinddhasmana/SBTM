...
describe('ProvisioningService', () => {
  ...
  describe('inviteUser', () => {
    ...
    it('throws BadRequest if user email already exists', async () => {
      ...
      await expect(
        service.inviteUser(
          { email: 'dup@test.example', role: Role.PARENT },
          mockOstaAdmin,
        ),
    ...
    it('prevents BOARD_ADMIN from inviting another BOARD_ADMIN', async () => {
      ...
      await expect(
        service.inviteUser(
          { email: 'new@test.example', role: Role.BOARD_ADMIN },
          mockBoardAdmin,
        ),
  ...
  describe('listUsers - tenant isolation', () => {
    ...
    it('throws ForbiddenException for DRIVER role', async () => {
      ...
      await expect(service.listUsers(driver)).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('deactivateUser', () => {
    ...
    it('BOARD_ADMIN cannot deactivate user from a different board', async () => {
      ...
      mockRepo.findOne.mockResolvedValue(target);
      await expect(
        service.deactivateUser('target-2', mockBoardAdmin),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('throws NotFoundException for unknown user', async () => {
      mockRepo.findOne.mockResolvedValue(null);
      await expect(
        service.deactivateUser('unknown', mockOstaAdmin),
