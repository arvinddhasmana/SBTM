...
export class OptimizationService {
    ...
    async optimizeStops(stops: CreateRouteStopDto[]): Promise<OptimizationResult> {
        if (stops.length < 2) {
            return this.buildFallbackResult(stops, 'Fewer than 2 stops — skipping provider call');
        }
        ...
        for (const stop of stops) {
            ...
            if (!parsed) {
                this.logger.warn(`Stop at sequence ${stop.sequence} has invalid or missing WKT location — falling back`);
                return this.buildFallbackResult(stops, 'One or more stops have invalid location WKT');
            }
            coords.push(parsed);
        }
        ...
        try {
            ...
            this.logger.warn(`OSRM call failed (${message}) — returning fallback result`);
     ...
     */
    private buildFallbackResult(stops: CreateRouteStopDto[], reason: string): OptimizationResult {
        this.logger.warn(`Using fallback optimization: ${reason}`);

        const optimized = [...stops].sort((a, b) => a.address.localeCompare(b.address));
        optimized.forEach((stop, index) => { stop.sequence = index; });
