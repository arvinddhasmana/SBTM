...
export class RouteService {
    ...
    async create(createRouteDto: CreateRouteDto): Promise<Route> {
        const { schoolId, name, vehicleId, startTime, stops } = createRouteDto;
        ...
        if (vehicleId) {
            await this.validateVehicleOverlap(vehicleId, startTime, createRouteDto.estimatedDuration || 60, null);
        }
        ...
        try {
            ...
            if (stops && stops.length > 0) {
                ...
                await queryRunner.manager.save(routeStops);
            }
    ...
    async update(id: string, schoolId: string, updateRouteDto: UpdateRouteDto): Promise<Route> {
        ...
        const newDuration = updateRouteDto.estimatedDuration ?? route.estimatedDuration;

        if (newVehicleId) {
            await this.validateVehicleOverlap(newVehicleId, newStartTime, newDuration, id);
        }
    ...
    private async validateVehicleOverlap(vehicleId: string, startTime: string, duration: number, currentRouteId: string | null) {
        ...
        for (const vRoute of vehicleRoutes) {
            ...
            if (newStart < existingEnd && newEnd > existingStart) {
                throw new ConflictException(`Vehicle already assigned to route ${vRoute.name} at this time`);
            }
        }
    }
