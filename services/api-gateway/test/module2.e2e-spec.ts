    ...
    beforeAll(async () => {
        const moduleFixture: TestingModule = await Test.createTestingModule({
            imports: [
                ...
                TypeOrmModule.forFeature([User, School, SchoolBoard, Vehicle, Route, RouteStop]),
        ...
        await authService.createUser('schoolA1@test.com', 'password', Role.SCHOOL_ADMIN, { boardId: board.id, schoolId: schoolA1Id });
        await authService.createUser('schoolA2@test.com', 'password', Role.SCHOOL_ADMIN, { boardId: board.id, schoolId: schoolA2Id });

        const login1 = await authService.login({ email: 'schoolA1@test.com', password: 'password' });
        schoolA1Token = login1.accessToken;

        const login2 = await authService.login({ email: 'schoolA2@test.com', password: 'password' });
