...
describe('Multi-Tenancy (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await authService.createUser(
      ...
      'password',
      Role.SCHOOL_ADMIN,
      { boardId: boardAId, schoolId: schoolA1Id },
    );

    // 3. Get Tokens
    const ostaLogin = await authService.login({
      email: 'osta@test.com',
      password: 'password',
    });
    ostaToken = ostaLogin.accessToken;

    const boardALogin = await authService.login({
      email: 'boardA@test.com',
      password: 'password',
    });
    boardAToken = boardALogin.accessToken;

    const schoolA1Login = await authService.login({
