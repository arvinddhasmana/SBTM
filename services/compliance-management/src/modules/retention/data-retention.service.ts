...
export class DataRetentionService {
    ...
    // ----- Presence Events: purge after 90 days -----
    @Cron('0 2 * * *')
    async purgeExpiredPresenceEvents(): Promise<void> {
        const cutoff = this.cutoffDate(90);
        try {
            const result = await this.dataSource.query(
                `DELETE FROM presence_event WHERE "createdAt" < $1`,
                [cutoff],
            );
            this.logger.log({
                level: 'info',
    ...
    // ----- Emergency Alerts: purge after 1 year -----
    @Cron('0 3 * * *')
    async purgeExpiredAlerts(): Promise<void> {
        const cutoff = this.cutoffDate(365);
        try {
            const result = await this.dataSource.query(
                `DELETE FROM emergency_alert WHERE "createdAt" < $1`,
                [cutoff],
            );
            this.logger.log({
                level: 'info',
