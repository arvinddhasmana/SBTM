...
export class DsarService {
  ...
  async fulfil(dto: DsarRequestDto, authenticatedSchoolId: string): Promise<DsarResponse> {
    ...
    const presenceEvents = await this.dataSource.query(
      `SELECT id, school_id AS "schoolId", vehicle_id AS "vehicleId",
                    route_id AS "routeId", event_kind AS "eventKind",
                    recorded_at AS timestamp, source, created_at AS "createdAt"
             FROM stx_boarding_events
