import { BadRequestException, Inject, Injectable, Logger, Optional } from '@nestjs/common';
import { CanonicalRecord } from '../adapter/types/canonical-record';
import { StubOsrmClient, type OsrmClient } from '../shape-fallback/osrm-client';
import { OSRM_CLIENT } from '../shape-fallback/osrm.provider';
import { PG_POOL, type PgQueryable } from '../staging/pg-pool.provider';
import { PII_CRYPTO, type PiiCrypto } from './pii-crypto.provider';

/**
 * Slice 2b (transport layer): reads stage_* rows for an `import_session_id`
 * and promotes them into the canonical v2 tables in FK-safe order.
 *
 * Scope (transport only):
 *   stx_sta → stx_boards → stx_schools → stx_operators → stx_vehicles →
 *   agency → calendar → routes → stops → shapes → trips → stop_times
 *
 * Deferred (PII layer): students / guardians / student_guardians / ridership.
 * These need the encryption hook before they are safe to write — tracked in
 * docs/Design/v2-followups.md item #8.
 *
 * Connection model: the service expects a Postgres connection that bypasses
 * RLS (superuser, or with `sbtm.user_anchor_kind=super` SET LOCAL). The
 * importer is an internal service; tenant-scoped clients never hit this code
 * path.
 *
 * Transactionality: a single BEGIN..COMMIT wraps every UPSERT. Any failure
 * rolls back; the import_session is marked `failed` by the caller.
 */
export interface CommitInput {
  importSessionId: string;
  /** Canonical STA short_code from manifest (also stored on import_sessions). */
  staShortCode: string;
  /** Display name; falls back to short_code if absent. */
  staName?: string;
}

export interface CommitCounts {
  sta: number;
  board: number;
  school: number;
  operator: number;
  vehicle: number;
  agency: number;
  calendar: number;
  route: number;
  stop: number;
  shape: number;
  trip: number;
  stopTime: number;
  /** Routes whose shape was generated by the OSRM fallback (slice 3). */
  shapesGenerated: number;
  /** Routes that STILL have no shape rows after the fallback ran. The
   * --verify-shapes CLI flag fails non-zero when this is > 0 so CI catches
   * "map renders blank" regressions. */
  zeroShapeRoutes: number;
  /** PII layer (slice 4). */
  student: number;
  guardian: number;
  studentGuardian: number;
  ridership: number;
}

interface StageRow<P> {
  payload: P;
}

@Injectable()
export class CommitService {
  private readonly logger = new Logger(CommitService.name);

  constructor(
    @Inject(PG_POOL) private readonly pg: PgQueryable,
    @Inject(OSRM_CLIENT) private readonly osrm: OsrmClient = new StubOsrmClient(),
    @Optional() @Inject(PII_CRYPTO) private readonly pii: PiiCrypto | null = null,
  ) {}

  async commit(input: CommitInput): Promise<CommitCounts> {
    // Guard: only commit sessions that have been successfully validated.
    const sessionCheck = await this.pg.query(`SELECT status FROM import_sessions WHERE id = $1`, [
      input.importSessionId,
    ]);
    if (!sessionCheck.rows.length) {
      throw new BadRequestException(`Import session ${input.importSessionId} not found`);
    }
    const sessionStatus = (sessionCheck.rows[0] as { status: string }).status;
    if (sessionStatus !== 'validated') {
      throw new BadRequestException(
        `Session ${input.importSessionId} is '${sessionStatus}', expected 'validated'`,
      );
    }

    const counts: CommitCounts = {
      sta: 0,
      board: 0,
      school: 0,
      operator: 0,
      vehicle: 0,
      agency: 0,
      calendar: 0,
      route: 0,
      stop: 0,
      shape: 0,
      trip: 0,
      stopTime: 0,
      shapesGenerated: 0,
      zeroShapeRoutes: 0,
      student: 0,
      guardian: 0,
      studentGuardian: 0,
      ridership: 0,
    };
    const committedRouteIds: string[] = [];

    await this.pg.query('BEGIN', []);
    try {
      const staId = await this.upsertSta(input.staShortCode, input.staName);
      counts.sta = 1;

      const boards = await this.readStage<Extract<CanonicalRecord, { kind: 'board' }>>(
        input.importSessionId,
        'stage_board_school',
        `school_code = ''`,
      );
      const boardIdByCode = new Map<string, string>();
      for (const { payload } of boards) {
        const id = await this.upsertBoard(staId, payload);
        boardIdByCode.set(payload.boardCode, id);
        counts.board += 1;
      }

      const schools = await this.readStage<Extract<CanonicalRecord, { kind: 'school' }>>(
        input.importSessionId,
        'stage_board_school',
        `school_code <> ''`,
      );
      const schoolIdByCode = new Map<string, string>();
      for (const { payload } of schools) {
        const boardId = boardIdByCode.get(payload.boardCode);
        if (!boardId) {
          throw new BadRequestException(
            `school ${payload.schoolCode} references unknown board ${payload.boardCode}`,
          );
        }
        const id = await this.upsertSchool(boardId, payload);
        schoolIdByCode.set(payload.schoolCode, id);
        counts.school += 1;
      }

      const operators = await this.readStage<Extract<CanonicalRecord, { kind: 'operator' }>>(
        input.importSessionId,
        'stage_operators',
      );
      const operatorIdByCode = new Map<string, string>();
      for (const { payload } of operators) {
        const id = await this.upsertOperator(payload);
        operatorIdByCode.set(payload.operatorCode, id);
        counts.operator += 1;
      }

      const vehicles = await this.readStage<Extract<CanonicalRecord, { kind: 'vehicle' }>>(
        input.importSessionId,
        'stage_vehicles',
      );
      for (const { payload } of vehicles) {
        const operatorId = operatorIdByCode.get(payload.operatorCode);
        if (!operatorId) {
          throw new BadRequestException(
            `vehicle ${payload.vehicleCode} references unknown operator ${payload.operatorCode}`,
          );
        }
        await this.upsertVehicle(operatorId, payload);
        counts.vehicle += 1;
      }

      const agencyId = await this.upsertAgency(staId, input.staShortCode, input.staName);
      counts.agency = 1;

      const trips = await this.readStage<Extract<CanonicalRecord, { kind: 'trip' }>>(
        input.importSessionId,
        'stage_trips',
      );
      const serviceIds = new Set<string>();
      for (const { payload } of trips) serviceIds.add(payload.serviceId);
      for (const serviceId of serviceIds) {
        await this.upsertWeekdayCalendar(serviceId);
        counts.calendar += 1;
      }

      const routes = await this.readStage<Extract<CanonicalRecord, { kind: 'route' }>>(
        input.importSessionId,
        'stage_routes',
      );
      for (const { payload } of routes) {
        const schoolId = schoolIdByCode.get(payload.schoolCode);
        if (!schoolId) {
          throw new BadRequestException(
            `route ${payload.staRouteNumber} references unknown school ${payload.schoolCode}`,
          );
        }
        await this.upsertRoute(staId, schoolId, agencyId, payload);
        committedRouteIds.push(payload.staRouteNumber);
        counts.route += 1;
      }

      const stops = await this.readStage<Extract<CanonicalRecord, { kind: 'stop' }>>(
        input.importSessionId,
        'stage_stops',
      );
      for (const { payload } of stops) {
        await this.upsertStop(payload);
        counts.stop += 1;
      }
      await this.snapStopsToRoads(stops.map((s) => s.payload.staStopId));

      const shapes = await this.readStage<Extract<CanonicalRecord, { kind: 'shape' }>>(
        input.importSessionId,
        'stage_shapes',
      );
      for (const { payload } of shapes) {
        await this.upsertShape(payload);
        counts.shape += 1;
      }

      for (const { payload } of trips) {
        await this.upsertTrip(payload);
        counts.trip += 1;
      }

      const stopTimes = await this.readStage<Extract<CanonicalRecord, { kind: 'stopTime' }>>(
        input.importSessionId,
        'stage_stop_times',
      );
      for (const { payload } of stopTimes) {
        await this.upsertStopTime(payload);
        counts.stopTime += 1;
      }

      // Slice 4: PII layer. Skipped when no PiiCrypto is provided (legacy
      // transport-only callers); when present, encrypt + write the four PII
      // tables in FK-safe order: students → guardians → student_guardians →
      // ridership.
      if (this.pii) {
        const studentIdByBoardNum = new Map<string, string>();
        const students = await this.readStage<Extract<CanonicalRecord, { kind: 'student' }>>(
          input.importSessionId,
          'stage_students',
        );
        for (const { payload } of students) {
          const schoolId = schoolIdByCode.get(payload.schoolCode);
          if (!schoolId) {
            throw new BadRequestException(
              `student ${payload.boardStudentNumber} references unknown school ${payload.schoolCode}`,
            );
          }
          const id = await this.upsertStudent(schoolId, payload, this.pii);
          studentIdByBoardNum.set(payload.boardStudentNumber, id);
          counts.student += 1;
        }

        const guardianIdByCode = new Map<string, string>();
        const guardians = await this.readStage<Extract<CanonicalRecord, { kind: 'guardian' }>>(
          input.importSessionId,
          'stage_guardians',
        );
        for (const { payload } of guardians) {
          const id = await this.upsertGuardian(payload, this.pii);
          guardianIdByCode.set(payload.guardianCode, id);
          counts.guardian += 1;
        }

        const links = await this.readStage<Extract<CanonicalRecord, { kind: 'studentGuardian' }>>(
          input.importSessionId,
          'stage_student_guardians',
        );
        for (const { payload } of links) {
          const studentId = studentIdByBoardNum.get(payload.boardStudentNumber);
          const guardianId = guardianIdByCode.get(payload.guardianCode);
          if (!studentId || !guardianId) {
            throw new BadRequestException(
              `student_guardian links unknown student=${payload.boardStudentNumber} guardian=${payload.guardianCode}`,
            );
          }
          await this.upsertStudentGuardian(studentId, guardianId, payload);
          counts.studentGuardian += 1;
        }

        const ridership = await this.readStage<Extract<CanonicalRecord, { kind: 'ridership' }>>(
          input.importSessionId,
          'stage_ridership',
        );
        for (const { payload } of ridership) {
          const studentId = studentIdByBoardNum.get(payload.boardStudentNumber);
          if (!studentId) {
            throw new BadRequestException(
              `ridership references unknown student ${payload.boardStudentNumber}`,
            );
          }
          counts.ridership += await this.upsertRidership(studentId, payload);
        }
      }

      await this.pg.query(
        `UPDATE import_sessions SET status = 'committed', updated_at = now() WHERE id = $1`,
        [input.importSessionId],
      );
      await this.pg.query('COMMIT', []);
    } catch (err) {
      await this.pg.query('ROLLBACK', []);
      throw err;
    }

    // Slice 3 post-processor: any committed route whose trip references a
    // shape_id with zero shapes rows gets a road-snapped polyline from OSRM.
    // Runs in its own transaction so a shape-fallback failure doesn't roll
    // back the (already-committed) transport entities.
    if (committedRouteIds.length > 0) {
      try {
        counts.shapesGenerated = await this.runShapeFallback(committedRouteIds);
      } catch (err) {
        this.logger.error(
          `shape fallback failed session=${input.importSessionId}: ${(err as Error).message}`,
        );
      }
      // After fallback, surface any route whose trips still have zero shape
      // rows — these would render as straight lines (or nothing) on the map
      // and are the exact failure mode the user reported. We don't throw
      // here so a partial-bundle commit still completes, but we log loud
      // enough that scripts/import-sample.ts --verify-shapes can pick it up
      // and fail the CLI exit code.
      const zeroShape = await this.pg.query(
        `SELECT DISTINCT t.route_id
           FROM trips t
          WHERE t.route_id = ANY($1::text[])
            AND (t.shape_id IS NULL
                 OR NOT EXISTS (SELECT 1 FROM shapes s WHERE s.shape_id = t.shape_id))`,
        [committedRouteIds],
      );
      const zeroShapeIds = (zeroShape.rows as { route_id: string }[]).map((r) => r.route_id);
      counts.zeroShapeRoutes = zeroShapeIds.length;
      if (zeroShapeIds.length > 0) {
        this.logger.error(
          `commit session=${input.importSessionId} ${zeroShapeIds.length} route(s) STILL have no shape after fallback: ${zeroShapeIds.join(', ')}`,
        );
      }
    }
    this.logger.log(`commit session=${input.importSessionId} counts=${JSON.stringify(counts)}`);
    return counts;
  }

  private async readStage<P>(
    sessionId: string,
    table: string,
    extraWhere?: string,
  ): Promise<StageRow<P>[]> {
    const where = extraWhere ? ` AND ${extraWhere}` : '';
    const res = await this.pg.query(
      `SELECT payload FROM ${table} WHERE import_session_id = $1${where} ORDER BY row_number`,
      [sessionId],
    );
    return res.rows as StageRow<P>[];
  }

  private async upsertSta(shortCode: string, name?: string): Promise<string> {
    const res = await this.pg.query(
      `INSERT INTO stx_sta (name, short_code)
       VALUES ($1, $2)
       ON CONFLICT (short_code) DO UPDATE
         SET name = EXCLUDED.name, updated_at = now()
       RETURNING id`,
      [name ?? shortCode, shortCode],
    );
    return (res.rows[0] as { id: string }).id;
  }

  private async upsertBoard(
    staId: string,
    r: Extract<CanonicalRecord, { kind: 'board' }>,
  ): Promise<string> {
    const res = await this.pg.query(
      `INSERT INTO stx_boards (sta_id, name, short_code, external_ids)
       VALUES ($1, $2, $3, $4::jsonb)
       ON CONFLICT (sta_id, short_code) DO UPDATE
         SET name = EXCLUDED.name, updated_at = now()
       RETURNING id`,
      [staId, r.boardName, r.boardCode, JSON.stringify({ board_code: r.boardCode })],
    );
    return (res.rows[0] as { id: string }).id;
  }

  private async upsertSchool(
    boardId: string,
    r: Extract<CanonicalRecord, { kind: 'school' }>,
  ): Promise<string> {
    // stx_schools has no unique constraint we can target with ON CONFLICT,
    // so we look up by external_ids->>'school_code' (scoped to this board)
    // and route to UPDATE or INSERT.
    const existing = await this.pg.query(
      `SELECT id FROM stx_schools
        WHERE board_id = $1 AND external_ids->>'school_code' = $2`,
      [boardId, r.schoolCode],
    );
    const externalIds = JSON.stringify({ school_code: r.schoolCode });

    if (existing.rows.length > 0) {
      const id = (existing.rows[0] as { id: string }).id;
      await this.pg.query(
        `UPDATE stx_schools
            SET name = $2,
                address = $3,
                location = ST_SetSRID(ST_MakePoint($4, $5), 4326)::geography,
                alerts_enabled = $6,
                updated_at = now()
          WHERE id = $1`,
        [id, r.schoolName, r.address, r.longitude, r.latitude, r.alertsEnabled],
      );
      return id;
    }

    const res = await this.pg.query(
      `INSERT INTO stx_schools
         (board_id, name, address, location, alerts_enabled, external_ids)
       VALUES
         ($1, $2, $3, ST_SetSRID(ST_MakePoint($4, $5), 4326)::geography, $6, $7::jsonb)
       RETURNING id`,
      [boardId, r.schoolName, r.address, r.longitude, r.latitude, r.alertsEnabled, externalIds],
    );
    return (res.rows[0] as { id: string }).id;
  }

  private async upsertOperator(r: Extract<CanonicalRecord, { kind: 'operator' }>): Promise<string> {
    const externalIds: Record<string, string> = { operator_code: r.operatorCode };
    if (r.legalEntityId) externalIds.legal_entity_id = r.legalEntityId;

    // Dedupe by legal_entity_id when present (cross-STA shared operators).
    if (r.legalEntityId) {
      const existing = await this.pg.query(
        `SELECT id FROM stx_operators WHERE external_ids->>'legal_entity_id' = $1`,
        [r.legalEntityId],
      );
      if (existing.rows.length > 0) {
        const id = (existing.rows[0] as { id: string }).id;
        await this.pg.query(
          `UPDATE stx_operators
             SET legal_name = $2,
                 contact_email = $3,
                 contact_phone = $4,
                 external_ids = external_ids || $5::jsonb,
                 updated_at = now()
           WHERE id = $1`,
          [id, r.legalName, r.contactEmail, r.contactPhone, JSON.stringify(externalIds)],
        );
        return id;
      }
    }

    const res = await this.pg.query(
      `INSERT INTO stx_operators (legal_name, contact_email, contact_phone, external_ids)
       VALUES ($1, $2, $3, $4::jsonb)
       RETURNING id`,
      [r.legalName, r.contactEmail, r.contactPhone, JSON.stringify(externalIds)],
    );
    return (res.rows[0] as { id: string }).id;
  }

  private async upsertVehicle(
    operatorId: string,
    r: Extract<CanonicalRecord, { kind: 'vehicle' }>,
  ): Promise<void> {
    const licensePlate = r.licensePlate ?? r.vehicleCode;
    const equipment = r.equipmentJson ?? '{}';
    await this.pg.query(
      `INSERT INTO stx_vehicles
         (operator_id, license_plate, capacity_seated, capacity_wheelchair, equipment, external_ids)
       VALUES ($1, $2, $3, $4, $5::jsonb, $6::jsonb)
       ON CONFLICT (operator_id, license_plate) DO UPDATE
         SET capacity_seated = EXCLUDED.capacity_seated,
             capacity_wheelchair = EXCLUDED.capacity_wheelchair,
             equipment = EXCLUDED.equipment,
             updated_at = now()`,
      [
        operatorId,
        licensePlate,
        r.capacitySeated,
        r.capacityWheelchair,
        equipment,
        JSON.stringify({ vehicle_code: r.vehicleCode }),
      ],
    );
  }

  private async upsertAgency(staId: string, shortCode: string, name?: string): Promise<string> {
    const agencyId = `AG-${shortCode}`;
    await this.pg.query(
      `INSERT INTO agency
         (agency_id, agency_name, agency_url, stx_agency_kind, stx_sta_id)
       VALUES ($1, $2, $3, 'sta', $4)
       ON CONFLICT (agency_id) DO UPDATE
         SET agency_name = EXCLUDED.agency_name, updated_at = now()`,
      [agencyId, name ?? shortCode, `https://sbtm.local/sta/${shortCode}`, staId],
    );
    return agencyId;
  }

  private async upsertWeekdayCalendar(serviceId: string): Promise<void> {
    await this.pg.query(
      `INSERT INTO calendar
         (service_id, monday, tuesday, wednesday, thursday, friday, saturday, sunday, start_date, end_date)
       VALUES ($1, true, true, true, true, true, false, false, '2025-09-02', '2026-06-26')
       ON CONFLICT (service_id) DO NOTHING`,
      [serviceId],
    );
  }

  private async upsertRoute(
    staId: string,
    schoolId: string,
    agencyId: string,
    r: Extract<CanonicalRecord, { kind: 'route' }>,
  ): Promise<void> {
    const direction = r.direction.toLowerCase();
    await this.pg.query(
      `INSERT INTO routes
         (route_id, agency_id, route_short_name, route_long_name, route_type,
          stx_sta_id, stx_school_id, stx_direction_kind)
       VALUES ($1, $2, $3, $4, 712, $5, $6, $7::stx_direction_kind_enum)
       ON CONFLICT (route_id) DO UPDATE
         SET agency_id = EXCLUDED.agency_id,
             route_long_name = EXCLUDED.route_long_name,
             stx_school_id = EXCLUDED.stx_school_id,
             stx_direction_kind = EXCLUDED.stx_direction_kind,
             updated_at = now()`,
      [r.staRouteNumber, agencyId, r.staRouteNumber, r.description, staId, schoolId, direction],
    );
  }

  private async upsertStop(r: Extract<CanonicalRecord, { kind: 'stop' }>): Promise<void> {
    // Canonical 'dropoff'/'depot' aren't in the DB enum yet — fold to 'pickup'.
    const dbKind =
      r.stopKind === 'school' ? 'school' : r.stopKind === 'pickup' ? 'pickup' : 'pickup';
    await this.pg.query(
      `INSERT INTO stops (stop_id, stop_name, stop_lat, stop_lon, stx_stop_kind, stx_hazard_zone)
       VALUES ($1, $2, $3, $4, $5::stx_stop_kind_enum, $6)
       ON CONFLICT (stop_id) DO UPDATE
         SET stop_name = EXCLUDED.stop_name,
             stop_lat = EXCLUDED.stop_lat,
             stop_lon = EXCLUDED.stop_lon,
             stx_stop_kind = EXCLUDED.stx_stop_kind,
             stx_hazard_zone = EXCLUDED.stx_hazard_zone,
             updated_at = now()`,
      [r.staStopId, r.name, r.latitude, r.longitude, dbKind, r.hazardZone],
    );
  }

  private async upsertShape(r: Extract<CanonicalRecord, { kind: 'shape' }>): Promise<void> {
    await this.pg.query(
      `INSERT INTO shapes (shape_id, shape_pt_lat, shape_pt_lon, shape_pt_sequence, shape_dist_traveled)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (shape_id, shape_pt_sequence) DO UPDATE
         SET shape_pt_lat = EXCLUDED.shape_pt_lat,
             shape_pt_lon = EXCLUDED.shape_pt_lon,
             shape_dist_traveled = EXCLUDED.shape_dist_traveled`,
      [r.shapeId, r.latitude, r.longitude, r.sequence, r.distTraveled],
    );
  }

  private async upsertTrip(r: Extract<CanonicalRecord, { kind: 'trip' }>): Promise<void> {
    await this.pg.query(
      `INSERT INTO trips
         (trip_id, route_id, service_id, shape_id, trip_headsign, direction_id, block_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       ON CONFLICT (trip_id) DO UPDATE
         SET route_id = EXCLUDED.route_id,
             service_id = EXCLUDED.service_id,
             shape_id = EXCLUDED.shape_id,
             trip_headsign = EXCLUDED.trip_headsign,
             direction_id = EXCLUDED.direction_id,
             block_id = EXCLUDED.block_id,
             updated_at = now()`,
      [r.staTripId, r.staRouteNumber, r.serviceId, r.shapeId, r.headsign, r.directionId, r.blockId],
    );
  }

  private async upsertStopTime(r: Extract<CanonicalRecord, { kind: 'stopTime' }>): Promise<void> {
    await this.pg.query(
      `INSERT INTO stop_times
         (trip_id, stop_sequence, arrival_time, departure_time, stop_id, stx_dwell_seconds)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (trip_id, stop_sequence) DO UPDATE
         SET arrival_time = EXCLUDED.arrival_time,
             departure_time = EXCLUDED.departure_time,
             stop_id = EXCLUDED.stop_id,
             stx_dwell_seconds = EXCLUDED.stx_dwell_seconds`,
      [
        r.staTripId,
        r.sequence,
        r.scheduledArrival,
        r.scheduledDeparture,
        r.staStopId,
        r.dwellSeconds,
      ],
    );
  }

  /**
   * Curb-snap pass: for each stop just upserted, call OSRM /nearest and
   * update stops.stop_lat / stop_lon to the snapped coords. Silently no-op
   * when the OSRM client doesn't support nearest() (StubOsrmClient does
   * return the same coord, which is a fine identity). Failures per stop are
   * logged and skipped so a partial OSRM outage doesn't roll back the commit.
   */
  async snapStopsToRoads(stopIds: string[]): Promise<void> {
    if (stopIds.length === 0 || !this.osrm.nearest) return;
    for (const stopId of stopIds) {
      const res = await this.pg.query(
        `SELECT stop_lat AS lat, stop_lon AS lon FROM stops WHERE stop_id = $1`,
        [stopId],
      );
      const row = res.rows[0] as { lat: number; lon: number } | undefined;
      if (!row) continue;
      try {
        const snapped = await this.osrm.nearest({
          lat: Number(row.lat),
          lon: Number(row.lon),
        });
        if (!snapped) continue;
        await this.pg.query(
          `UPDATE stops SET stop_lat = $2, stop_lon = $3, updated_at = now() WHERE stop_id = $1`,
          [stopId, snapped.lat, snapped.lon],
        );
      } catch (err) {
        this.logger.warn(`stop snap failed for ${stopId}: ${(err as Error).message}`);
      }
    }
  }

  /**
   * Slice 3 post-processor. For each route_id, finds trips whose shape_id is
   * NULL or references zero rows in `shapes`. For each such trip:
   *  - synthesize `gen-{route_id}` as the shape_id when the trip has none,
   *  - road-snap the trip's stop sequence via OSRM,
   *  - INSERT generated shape rows,
   *  - UPDATE the trip's shape_id when it was null,
   *  - flag the route as `stx_shape_source = 'sbtm_generated'`.
   * Returns the count of routes whose shape was generated.
   */
  async runShapeFallback(routeIds: string[]): Promise<number> {
    if (routeIds.length === 0) return 0;
    const missing = await this.pg.query(
      `SELECT t.route_id, t.trip_id, t.shape_id
         FROM trips t
        WHERE t.route_id = ANY($1::text[])
          AND (t.shape_id IS NULL
               OR NOT EXISTS (SELECT 1 FROM shapes s WHERE s.shape_id = t.shape_id))`,
      [routeIds],
    );
    const rows = missing.rows as {
      route_id: string;
      trip_id: string;
      shape_id: string | null;
    }[];
    if (rows.length === 0) return 0;

    let generated = 0;
    await this.pg.query('BEGIN', []);
    try {
      const seenShapes = new Set<string>();
      const seenRoutes = new Set<string>();
      for (const row of rows) {
        const shapeId = row.shape_id ?? `gen-${row.route_id}`;
        if (!seenShapes.has(shapeId)) {
          const stopsRes = await this.pg.query(
            `SELECT s.stop_lat AS lat, s.stop_lon AS lon
               FROM stop_times st
               JOIN stops s ON s.stop_id = st.stop_id
              WHERE st.trip_id = $1
              ORDER BY st.stop_sequence`,
            [row.trip_id],
          );
          const coords = (stopsRes.rows as { lat: number; lon: number }[]).map((c) => ({
            lat: Number(c.lat),
            lon: Number(c.lon),
          }));
          if (coords.length < 2) {
            this.logger.warn(
              `shape fallback: trip ${row.trip_id} has <2 stops, skipping shape ${shapeId}`,
            );
            continue;
          }
          const points = await this.osrm.routeSnappedPath(coords);
          for (const p of points) {
            await this.pg.query(
              `INSERT INTO shapes
                 (shape_id, shape_pt_lat, shape_pt_lon, shape_pt_sequence, shape_dist_traveled)
               VALUES ($1, $2, $3, $4, $5)
               ON CONFLICT (shape_id, shape_pt_sequence) DO NOTHING`,
              [shapeId, p.shapePtLat, p.shapePtLon, p.shapePtSequence, p.shapeDistTraveled],
            );
          }
          seenShapes.add(shapeId);
        }
        if (row.shape_id === null) {
          await this.pg.query(
            `UPDATE trips SET shape_id = $2, updated_at = now() WHERE trip_id = $1`,
            [row.trip_id, shapeId],
          );
        }
        if (!seenRoutes.has(row.route_id)) {
          await this.pg.query(
            `UPDATE routes SET stx_shape_source = 'sbtm_generated', updated_at = now() WHERE route_id = $1`,
            [row.route_id],
          );
          seenRoutes.add(row.route_id);
          generated += 1;
        }
      }
      await this.pg.query('COMMIT', []);
      this.logger.log(`shape fallback generated shapes for ${generated} route(s)`);
      return generated;
    } catch (err) {
      await this.pg.query('ROLLBACK', []);
      throw err;
    }
  }

  /**
   * Slice 4 — students. Dedupe key: external_ids->>'board_student_number'
   * within the same school_id. The board student number is stored in two
   * places: encrypted in the BYTEA column for read-back, and plaintext in
   * `external_ids` so we can re-find the row on re-import. (Stable IDs across
   * academic years come from the source system; collisions on the same
   * board_student_number → same student.)
   */
  private async upsertStudent(
    schoolId: string,
    r: Extract<CanonicalRecord, { kind: 'student' }>,
    pii: PiiCrypto,
  ): Promise<string> {
    const externalIds: Record<string, string> = {
      board_student_number: r.boardStudentNumber,
    };
    if (r.oen) externalIds.oen = r.oen;

    const existing = await this.pg.query(
      `SELECT id FROM stx_students
        WHERE school_id = $1 AND external_ids->>'board_student_number' = $2`,
      [schoolId, r.boardStudentNumber],
    );

    const boardNumCt = pii.encrypt(r.boardStudentNumber);
    const legalNameCt = pii.encrypt(r.legalName);
    const preferredCt = pii.encrypt(r.preferredName);
    const dobCt = pii.encrypt(r.dateOfBirth);
    const addressCt = pii.encrypt(r.homeAddress);

    if (existing.rows.length > 0) {
      const id = (existing.rows[0] as { id: string }).id;
      await this.pg.query(
        `UPDATE stx_students
            SET board_student_number = $2,
                legal_name = $3,
                preferred_name = $4,
                grade = $5,
                date_of_birth = $6,
                home_address = $7,
                home_location = ST_SetSRID(ST_MakePoint($8, $9), 4326)::geography,
                external_ids = external_ids || $10::jsonb,
                updated_at = now()
          WHERE id = $1`,
        [
          id,
          boardNumCt,
          legalNameCt,
          preferredCt,
          r.grade,
          dobCt,
          addressCt,
          r.homeLon,
          r.homeLat,
          JSON.stringify(externalIds),
        ],
      );
      return id;
    }

    const res = await this.pg.query(
      `INSERT INTO stx_students
         (school_id, board_student_number, legal_name, preferred_name, grade,
          date_of_birth, home_address, home_location, external_ids)
       VALUES
         ($1, $2, $3, $4, $5, $6, $7,
          ST_SetSRID(ST_MakePoint($8, $9), 4326)::geography, $10::jsonb)
       RETURNING id`,
      [
        schoolId,
        boardNumCt,
        legalNameCt,
        preferredCt,
        r.grade,
        dobCt,
        addressCt,
        r.homeLon,
        r.homeLat,
        JSON.stringify(externalIds),
      ],
    );
    return (res.rows[0] as { id: string }).id;
  }

  /**
   * Slice 4 — guardians. Dedupe across STAs / boards by
   * `external_ids->>'guardian_code'`. The two-STA sample bundle uses this to
   * land a single guardian row for parent O-P2 (one child OCSB + one child
   * OCDSB) with two `stx_student_guardians` link rows downstream.
   */
  private async upsertGuardian(
    r: Extract<CanonicalRecord, { kind: 'guardian' }>,
    pii: PiiCrypto,
  ): Promise<string> {
    const externalIds = { guardian_code: r.guardianCode };

    const existing = await this.pg.query(
      `SELECT id FROM stx_guardians WHERE external_ids->>'guardian_code' = $1`,
      [r.guardianCode],
    );

    const legalNameCt = pii.encrypt(r.legalName);
    const emailCt = pii.encrypt(r.email);
    const phoneCt = pii.encrypt(r.phone);

    if (existing.rows.length > 0) {
      const id = (existing.rows[0] as { id: string }).id;
      await this.pg.query(
        `UPDATE stx_guardians
            SET legal_name = $2,
                email = $3,
                phone = $4,
                preferred_language = $5,
                external_ids = external_ids || $6::jsonb,
                updated_at = now()
          WHERE id = $1`,
        [id, legalNameCt, emailCt, phoneCt, r.preferredLanguage, JSON.stringify(externalIds)],
      );
      return id;
    }

    const res = await this.pg.query(
      `INSERT INTO stx_guardians
         (legal_name, email, phone, preferred_language, external_ids)
       VALUES ($1, $2, $3, $4, $5::jsonb)
       RETURNING id`,
      [legalNameCt, emailCt, phoneCt, r.preferredLanguage, JSON.stringify(externalIds)],
    );
    return (res.rows[0] as { id: string }).id;
  }

  private async upsertStudentGuardian(
    studentId: string,
    guardianId: string,
    r: Extract<CanonicalRecord, { kind: 'studentGuardian' }>,
  ): Promise<void> {
    await this.pg.query(
      `INSERT INTO stx_student_guardians
         (student_id, guardian_id, relationship, is_primary_pickup)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (student_id, guardian_id) DO UPDATE
         SET relationship = EXCLUDED.relationship,
             is_primary_pickup = EXCLUDED.is_primary_pickup`,
      [studentId, guardianId, r.relationship, r.isPrimaryPickup],
    );
  }

  /**
   * Canonical ridership rows are keyed by (route, stop, direction) — they
   * describe how a student rides "the route", not a single trip. For each
   * matching trip on that route+direction we insert one stx_ridership row so
   * the audience resolver can later look up "who is on trip T?".
   * Returns the number of rows written for this canonical row.
   */
  private async upsertRidership(
    studentId: string,
    r: Extract<CanonicalRecord, { kind: 'ridership' }>,
  ): Promise<number> {
    const trips = await this.pg.query(
      `SELECT trip_id FROM trips WHERE route_id = $1 AND direction_id = $2`,
      [r.staRouteNumber, r.directionId],
    );
    let written = 0;
    for (const row of trips.rows as { trip_id: string }[]) {
      const existing = await this.pg.query(
        `SELECT id FROM stx_ridership
          WHERE student_id = $1 AND trip_id = $2 AND stop_id = $3 AND direction_id = $4`,
        [studentId, row.trip_id, r.staStopId, r.directionId],
      );
      if (existing.rows.length > 0) {
        await this.pg.query(
          `UPDATE stx_ridership
              SET effective_from = $2,
                  effective_to = $3,
                  status = 'active',
                  updated_at = now()
            WHERE id = $1`,
          [(existing.rows[0] as { id: string }).id, r.effectiveFrom, r.effectiveTo],
        );
      } else {
        await this.pg.query(
          `INSERT INTO stx_ridership
             (student_id, trip_id, stop_id, direction_id, effective_from, effective_to, status)
           VALUES ($1, $2, $3, $4, $5, $6, 'active')`,
          [studentId, row.trip_id, r.staStopId, r.directionId, r.effectiveFrom, r.effectiveTo],
        );
      }
      written += 1;
    }
    return written;
  }
}
