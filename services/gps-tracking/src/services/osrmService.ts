...
const OSRM_SNAP_TIMEOUT_MS = 2_000;
...
export class OsrmService {
  ...
  async snapToRoad(lat: number, lng: number): Promise<OsrmSnapResult | null> {
    try {
      ...
      const response = await axios.get(url, { timeout: OSRM_SNAP_TIMEOUT_MS });
  ...
  async getSnappedRoute(coordinates: [number, number][]): Promise<[number, number][] | null> {
    try {
      ...
      const response = await axios.get(url, { timeout: OSRM_ROUTE_TIMEOUT_MS });
