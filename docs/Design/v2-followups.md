...
- **Status**: partial — shared `AuthenticatedUser` type landed at `services/api-gateway/src/modules/auth/types/authenticated-user.ts` and wired into `JwtStrategy.validate()`. Controller migrations done so far: `video.controller.ts`, `organization.controller.ts`, `absence.controller.ts`, `fleet-assignment.controller.ts`, `provisioning.controller.ts`, `alerts.controller.ts`, `school.controller.ts`, `system-settings.controller.ts` (drop local v1-shaped `AuthenticatedRequest`, switch to `{ user: AuthenticatedUser }`, scope reads off `anchorKind`+`anchorId`). Gateway-service migrations done: `presence.gateway.service.ts` (4 methods), `student.gateway.service.ts` (6 methods, with explicit "school anchor required" Forbidden on SCHOOL_ADMIN paths that previously silently sent `undefined`; BOARD_ADMIN no longer attempts school-filter derivation from a board anchor). Shared helper `services/api-gateway/src/modules/auth/utils/anchor-scope.ts` exposes `schoolIdFromAnchor(user)` / `boardIdFromAnchor(user)` so future callsites collapse the projection to one line. `MultiTenancyGuard` also migrated to anchor reads. v1 `user.schoolId` reads are now **fully eliminated**: `notification-settings.controller.ts` and `parent.gateway.service.ts.getNotificationsForParent` were migrated in the same slice that landed #6's option-D rewrite (see #6 for the data-model change). The controller now reads `{ user: AuthenticatedUser }` only; all 5 prior `user.schoolId` reads are gone, and `getNotificationsForParent` forwards `{ parentUserId }` to alerts — audience resolution lives downstream in `stx_alert_subscriptions` so cross-board guardians get a single feed. `gps.gateway.service.ts` is now **done**: all 8 v1 callsites (`schoolId` / `assignedRouteIds` / `childRouteIds`) are gone, replaced by anchor-scoped DB resolvers (`stx_student_guardians` → `stx_ridership` → `trips` for parents; `stx_runs` → `trips` for drivers); every read wrapped in `rlsContext.runAsCurrent`. The previously-unused `RlsContextService` injection bug is closed in the same slice. `GpsController` switched to `AuthenticatedUser` typing across all 8 handlers; `GpsRouteAccessGuard.canActivate` is now async + typed. Specs rewritten — `gps.gateway.service.spec.ts` (19 tests: admin short-circuit, parent ridership match/miss, driver run match/miss, broken-anchor reject, alert-status enrichment, NotFound → `{active:false}`, ingest RBAC matrix, lifecycle-event RBAC + route-access gate, active-routes empty short-circuit, reference-route NotFound/hit) and `gps-route-access.guard.spec.ts` (5 tests: allow / Forbidden / skip-when-no-routeId / admin pass-through / driver wrong-route). Per-service callsite migration for the remaining stubs is folded into item #2 (each 501 stub rewrite drops its local `RequestUser` interface and switches reads to anchor helpers).
...
- **Status**: in progress on `feat/sbtm-refocus-data-model`. `DriverGatewayService.getScheduleForDriver` is wired against `stx_runs` + GTFS (`trips`/`routes`/`stop_times`/`stx_schools`) inside `rlsContext.runAsCurrent`; serves as the worked example for the remaining stubs. `DriverGatewayService.getRouteStudents` is now wired against `stx_ridership` joined to `stops`/`stop_times`/`trips`, scoped to trips on the caller's runs today, with student names decrypted via the `PII_CRYPTO` provider (mirrors importer's wiring; requires `SBTM_PII_KEY`). `AbsenceGatewayService.*` is now wired against `stx_student_absences`: parents report (with `stx_student_guardians` ownership check + duplicate guard), school admins confirm/reject (scoped via `stx_students.school_id`), board / sta / super admins list and delete with cascading scope joins through `stx_schools.board_id` / `stx_boards.sta_id`, drivers see absences for students on their `stx_runs` for the day via `stx_ridership`. Spec covers 24 cases including cross-scope denials and not-found / bad-state branches. `AlertsController.getParentAlertHistory` (the `parent.gateway.service.parent-history` slot) now derives the parent's active `route_id`s server-side via `AlertsGatewayService.getParentAlertHistory(guardianId)`: a single query through `stx_student_guardians → stx_ridership → trips` returns distinct active-today routes, then proxies to the alerts service's `/by-routes` endpoint; empty ridership short-circuits to `[]` so the dashboard renders cleanly. **Decisions 2026-05-17 (user)**: (a) `FleetAssignmentGatewayService.*` is **parked indefinitely** — the propose/accept/reject workflow is not part of the v2 surface; controller routes stay registered and throw a well-formed 501. No `stx_run_proposals` table will be added. (b) `DocumentController.*` is **skipped for now**, may be revisited later for compliance/audit; existing 501s stay. (c) `RouteService.create/update` is now **done on `feat/sbtm-refocus-data-model`**: a single `dataSource.transaction` writes one `routes` row + N `trips` + N×M `stop_times` + (optionally) `shapes`. STA anchor is resolved server-side via School→Board join; stops are reused-by-id or minted as PICKUP stops scoped to the school; `shape_source` is set from inputs (`STA_IMPORT` when `shapeId` reused, `STA_ADMIN_EDITED` when admin draws shapePoints, `SBTM_GENERATED` otherwise so OSRM fallback claims the slot). `update` is replace-in-place for trips: partial name/direction patches only touch the routes row; any `trips`/`stops`/`startTime` change requires the full stop list and triggers a delete-then-recreate of `stop_times` and `trips` for that route inside the same tx. Spec covers 19 cases including single-trip vs multi-trip, shape-source matrix, partial vs full update, and out-of-scope NotFound. Remaining stubs: `DocumentController.*` (deferred).
- **Student↔stop attachment model (decided 2026-05-17)**: canonical link is `stx_ridership.stop_id` (already present). `getRouteStudents(routeId, direction?, serviceDate?)` resolves via `stx_ridership` → `stops` → `stx_students`, scoped by `rlsContext.runAsCurrent` and additionally filtered to runs the driver has access to. Route Planner edits to stop location/order do **not** mutate `stx_ridership`; instead they emit `stx_student_stop_warnings` rows for STA Admin review (see `docs/Design/RoutePlanner.md` §9). This preserves the user's invariant that Planner is a geometry tool, not a roster tool.
...
- **Where** (api-gateway):
  ...
  - **Done**: `provisioning.service.spec.ts` rewritten against the v2 anchor model (23 tests covering invite scope rules, anchor resolution, activation lifecycle, anchor-scoped list/deactivate/reactivate, and the "no cross-anchor escalation" invariant); v1bak deleted.
  - **Done**: `driver.gateway.service.spec.ts.v1bak` deleted — the v2 `driver.gateway.service.spec.ts` already covers the equivalent surface (10 tests, anchor-model + `rlsContext.runAsCurrent`); v1 cases targeted now-deleted v1 implementation details.
  - **Done**: `parent.gateway.service.spec.ts` rewritten against v2 entity paths (`organization/School`, `gtfs/Route` with `routeId` PK + `routeShortName`/`routeLongName`); merged the v1 main + status specs into a single file (14 tests covering proxy/enrich, deterministic avatar, status derivation incl. null-direction PM-by-ID fallback, presence-query throw fallback, alerts notifications proxy, SSE smoke); both v1baks deleted. The legacy `user.schoolId` path in `getNotificationsForParent` is mirrored (not migrated) — it's tracked under #1.
  - **Done**: `fleet-assignment.gateway.service.spec.ts` rewritten as a phase-B 501 contract test (5 tests asserting `NotImplementedException` on all surfaces); v1bak deleted. Blocked on the `stx_run_proposals` lifecycle decision (see #2). All `services/api-gateway/**/*.v1bak` files are now resolved.
...
- **Option-D follow-on (done same slice)**: dropped `device_tokens.school_id` entirely and made `boardId` / `schoolId` optional throughout the notification pipeline so the four `stx_alerts.scope_kind` values (`sta` / `board` / `school` / `route`) all serialise cleanly. Migration `services/api-gateway/migrations/20260518_notification_optional_school.sql` drops `device_tokens.school_id`, makes `notification_delivery_log.school_id` nullable, and adds `notification_delivery_log.board_id uuid` + index. Entity / DTO / service / controller / processor / router updates land in the same commit; specs cover STA-scope (`schoolId=null, boardId=null`) and board-scope (`boardId set, schoolId=null`) delivery-log writes. Closes the last v1 `user.schoolId` reads in api-gateway (see #1).
- **Size**: closed.
