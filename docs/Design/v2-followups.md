...
- **Status**: in progress on `feat/sbtm-refocus-data-model`. `DriverGatewayService.getScheduleForDriver` is wired against `stx_runs` + GTFS (`trips`/`routes`/`stop_times`/`stx_schools`) inside `rlsContext.runAsCurrent`; serves as the worked example for the remaining stubs. `DriverGatewayService.getRouteStudents` and the rest still 501 pending per-stub design.
