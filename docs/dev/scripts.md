...
│   ├── init-db.sh               Entry point - creates schema and seeds data
...
**scripts/init-db.sh** (wrapper → schema-seed/init-db.sh)
...
```bash
./scripts/reset-demo-db.sh           # Reset database with demo data
./scripts/SimulationOnlyOnSeededDB.sh & # Start GPS simulation
./scripts/dev-hybrid.sh              # Start services
```
