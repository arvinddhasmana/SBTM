#!/usr/bin/env bash
# =============================================================================
# SBTM Dynamic Single-Bus Demo Simulator
# Features: 15 Students, 10 Parents, 1 Bus, 8 stops, AM/PM Routes,
#           OSRM-generated polylines, alerts with bus halt on critical.
# =============================================================================
set -euo pipefail
...
CONFIG_PATH="$SCRIPT_DIR/dynamic-config.json"
...
echo -e "\033[36m=== SBTM Dynamic Single-Bus Simulator ===\033[0m"
echo ""

read -rp "Enter school latitude [default: 45.3876]: " SCHOOL_LAT
SCHOOL_LAT="${SCHOOL_LAT:-45.3876}"

read -rp "Enter school longitude [default: -75.696]: " SCHOOL_LNG
SCHOOL_LNG="${SCHOOL_LNG:--75.696}"

read -rp "Enter school name [default: Dynamic School]: " SCHOOL_NAME
SCHOOL_NAME="${SCHOOL_NAME:-Dynamic School}"

echo ""
echo -e "\033[36mSchool: $SCHOOL_NAME at ($SCHOOL_LAT, $SCHOOL_LNG)\033[0m"
echo ""

# --- Generate Config via OSRM ---

echo -e "\033[36mGenerating route config via OSRM...\033[0m"
npx tsx "$SCRIPT_DIR/dynamic-generate-config.ts" "$SCHOOL_LAT" "$SCHOOL_LNG" "$SCHOOL_NAME"

if [ ! -f "$CONFIG_PATH" ]; then
  echo "Error: Config generation failed — $CONFIG_PATH not found"
  exit 1
fi

echo -e "\033[32mConfig generated successfully!\033[0m"
echo ""
...
# Create dynamic simulation users (driver + parents)..."
echo "Creating dynamic simulation vehicle and users..."
BOARD_ID=$(node_config "config.school.boardId")

# Update school name/coordinates (reuses existing school ID from init-db.sql)
SCHOOL_NAME_ESCAPED=$(node_config "config.school.name.replace(/'/g, \"''\")")
docker exec "sbtm-postgres-1" psql -U postgres -d sbms -c "
  UPDATE schools SET name = '$SCHOOL_NAME_ESCAPED' WHERE id = '$SCHOOL_ID';" > /dev/null 2>&1 || true

# Create vehicle record
docker exec "sbtm-postgres-1" psql -U postgres -d sbms -c "
  INSERT INTO vehicles (id, \"schoolId\", \"licensePlate\", status) VALUES ('$VEHICLE_ID', '$SCHOOL_ID', 'ON-2001', 'ACTIVE')
  ON CONFLICT (id) DO UPDATE SET \"schoolId\" = EXCLUDED.\"schoolId\";
  INSERT INTO vehicles_reference (id, \"plateNumber\", capacity, status) VALUES ('$VEHICLE_ID', 'ON-2001', 50, 'ACTIVE')
  ON CONFLICT (id) DO UPDATE SET status = EXCLUDED.status;" > /dev/null 2>&1 || true

# Create driver and parent users
node -e "
  const fs = require('fs');
  const config = JSON.parse(fs.readFileSync('$CONFIG_PATH', 'utf8'));
  const schoolId = config.school.id;
  const boardId = config.school.boardId;
  const routesCsv = config.am.routeId + ',' + config.pm.routeId;

  // Driver user
  const b = config.bus;
  process.stdout.write(\"INSERT INTO users (id, email, \\\"passwordHash\\\", role, \\\"firstName\\\", \\\"lastName\\\", \\\"driverId\\\", \\\"assignedRouteIds\\\", \\\"schoolId\\\", \\\"boardId\\\") VALUES ('20000000-0000-0000-0000-000000000100', '\" + b.driverEmail + \"', crypt('Admin123!', gen_salt('bf')), 'DRIVER', 'Dynamic', 'Driver', '\" + b.driverId + \"', '\" + routesCsv + \"', '\" + schoolId + \"', '\" + boardId + \"') ON CONFLICT (id) DO UPDATE SET \\\"assignedRouteIds\\\" = EXCLUDED.\\\"assignedRouteIds\\\", \\\"schoolId\\\" = EXCLUDED.\\\"schoolId\\\";\n\");

  // Parent users
  for (const parent of config.parents) {
    process.stdout.write(\"INSERT INTO users (id, email, \\\"passwordHash\\\", role, \\\"firstName\\\", \\\"lastName\\\", \\\"childRouteIds\\\", \\\"schoolId\\\", \\\"boardId\\\") VALUES ('\" + parent.userId + \"', '\" + parent.email + \"', crypt('Admin123!', gen_salt('bf')), 'PARENT', '\" + parent.firstName + \"', '\" + parent.lastName + \"', '\" + routesCsv + \"', '\" + schoolId + \"', '\" + boardId + \"') ON CONFLICT (id) DO UPDATE SET \\\"childRouteIds\\\" = EXCLUDED.\\\"childRouteIds\\\", \\\"schoolId\\\" = EXCLUDED.\\\"schoolId\\\";\n\");
  }
...
sync_route "$AM_ROUTE_ID" "Dynamic AM" "AM" "$AM_POLYLINE"
sync_route "$PM_ROUTE_ID" "Dynamic PM" "PM" "$PM_POLYLINE"
...
echo -e "\033[36mStarting Dynamic Single-Bus Simulation (Interval: ${INTERVAL_SECONDS}s)...\033[0m"
echo -e "\033[33mNote: During PM route, the bus will HALT on a critical alert.\033[0m"
echo -e "\033[33m      Resolve it on the Dashboard to resume the simulation.\033[0m"
echo ""
echo -e "\033[36mLogin credentials for this simulation:\033[0m"
echo -e "  Admin Dashboard: school.admin@sbtm.demo / Admin123!"
echo -e "  Parent Portal:   dparent1@sbtm.demo    / Admin123!"
echo -e "  (Parent emails:  dparent1 through dparent10 @sbtm.demo)"
echo ""
npx tsx scripts/dynamic-run.ts "$INTERVAL_SECONDS"

echo -e "\033[32mDynamic Simulation Finished Successfully!\033[0m"
